...
namespace Blaze.Interaction.Internal
{
    public sealed class LengthPrefixedFrameDecoder
    {
        ...
        public LengthPrefixedFrameDecoder(int maximumPayloadLength = InteractionIpcProtocol.MaximumPayloadLength)
        {
        ...
        public int BufferedByteCount { get { return _buffer.Count; } }
        ...
        public IReadOnlyList<byte[]> Append(byte[] bytes, int offset, int count)
        {
            ...
            while (_buffer.Count >= 4)
            {
                ...
                {
                    _buffer.Clear();
                    throw new InvalidDataException("Interaction IPC payload length is outside the allowed range.");
                }
