...
从旧版本升级必须先移除旧 Git URL，重新解析后在 Package Manager 确认 Blaze Radar SDK `1.2.8` 的 Resolved Path 指向当前 `Library/PackageCache/com.blaze.radar@...`。
...
1.2.8 在 Bridge 雷达参数中提供沿有效拉框真实边缘计算的四边死区，用于过滤三面墙/地面交线回波；屏幕参数中的“快速移动预设”会按逻辑分辨率提高跟踪关联距离并降低稀疏点门槛，点击后仍须保存并应用配置。
