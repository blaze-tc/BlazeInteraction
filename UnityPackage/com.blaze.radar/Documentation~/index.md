...
Remove any old Git URL before adding this URL. Select Blaze Radar SDK in Package Manager and confirm version `1.2.10`; its Resolved Path must be the newly resolved `Library/PackageCache/com.blaze.radar@...`, not an old cache or local override. If Unity remains stale, close the Editor, remove only this package's cache entry and `Packages/packages-lock.json` entry, then reopen and resolve the tagged URL.
...
Bridge logs use `[SCREEN/SENSOR]` tags. Match their timestamps with `Player.log` fields for SDK/Bridge/IPC versions, screenId, batch/frame sequence, pointer count, dropped count and latency. IPC v1 and v2 are incompatible: close all old Bridge processes, remove the old package URL/cache, reinstall the tag, verify both versions are 1.2.10/IPC 2, then reconnect. Floating-point fields in the right panel accept both `0.15` and `0,15` and preserve a trailing separator while the value is being typed. Tolerated detection gaps retain the last target/pointer position; at 60 Hz use 10–12 lost ticks for roughly 167–200 ms of hold time.
