...
Remove any old Git URL before adding this URL. Select Blaze Radar SDK in Package Manager and confirm version `1.2.5`; its Resolved Path must be the newly resolved `Library/PackageCache/com.blaze.radar@...`, not an old cache or local override. If Unity remains stale, close the Editor, remove only this package's cache entry and `Packages/packages-lock.json` entry, then reopen and resolve the tagged URL.
