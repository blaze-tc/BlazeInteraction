...
本文说明 Blaze Radar SDK 与 `RadarBridge.exe` 的安装、连接、界面按钮、参数含义，以及 Unity Sample 的验证方法。适用版本：`1.2.7`，IPC 协议：`2`。
...
4. 选中 **Blaze Radar SDK**，确认版本为 `1.2.7`。
...
![RadarBridge 1.2.7 主界面，屏幕参数页展开](images/radarbridge-overview.png)

> 图 1：`RadarBridge 1.2.7` 默认窗口，右侧已展开“屏幕参数”。截图中的 `Unity 连接: False` 和 `Stopped` 表示截图时没有连接 Unity 或真实雷达，仅用于说明界面布局；现场运行时应以右上角连接状态、左侧雷达状态和底部日志为准。

界面从左到右分为五个功能区。`1.2.7` 将单雷达视图明确拆为 `1A 原始点观察` 与 `1B 拉框过滤结果`，提供独立的大画布拉框编辑器，并让参数正文铺满面板；页签标题保持居中，滚动条固定在参数面板最右侧。
...
- 点击区域 1 右上角 **放大编辑** 可打开独立大画布。编辑器同时叠加灰色原始点和青色过滤点；绿色顶点仍用于修改有效区域。
...
参数正文应横向铺满右侧面板：宽度/高度各占一列，其余单值输入框占满可用宽度；内容超出可见高度时，使用面板最右侧的垂直滚动条继续查看。若正文缩在中间、滚动条不在最右侧或页签出现白底浅字，说明运行的不是 `1.2.7` 完整包，应检查 Package Manager Version、Resolved Path 和 `bridge-version.txt`。
