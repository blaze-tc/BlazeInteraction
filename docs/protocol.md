...
- Bridge 校验非空/唯一 ID、合法分辨率/order 和恰好一个 Primary，应用 topology 后返回 `HelloAck`：Bridge `1.2.1`、protocol 2、capability `multi-screen` 和最终 screen summaries。
...
恢复顺序：停止 Player/Play Mode → 关闭旧 Bridge → 移除旧 Package URL/cache → 安装 `https://github.com/blaze-tc/RadarControl.git?path=/UnityPackage/com.blaze.radar#v1.2.1` → 确认 package/SDK/Bridge `1.2.1`、Resolved Path 和 `bridge-version.txt` → 重新握手并检查 HelloAck protocol 2/screen summaries。Pipe Name 不应用来绕过版本校验。
