...
- Bridge：`BridgeVersion.Value = 1.2.1`，WPF footer 为 `Bridge 1.2.1 · IPC 2 · Windows x64`。
- Unity SDK：`UnitySdkVersion.Value = 1.2.1`；`package.json` 与 `bridge-version.txt` 同为 `1.2.1`。
...
- 安装 URL：`https://github.com/blaze-tc/RadarControl.git?path=/UnityPackage/com.blaze.radar#v1.2.1`。
...
- 1.2.1 不自动完成联合标定；每个雷达仍由物理四角与 OutputRect 对齐。标定和屏蔽区依赖现场几何。
