...
从旧版本升级时，先从 `Packages/manifest.json` 删除旧的 Git URL，再安装上述 URL；安装后在 Package Manager 中选择 Blaze Radar SDK，确认版本为 `1.2.5`，Resolved Path 指向本次解析的 `Library/PackageCache/com.blaze.radar@...`，而不是旧缓存或本地覆盖目录。完整步骤见 [安装、升级与现场验收](INSTALL.md)。
